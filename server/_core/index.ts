...
async function startServer() {
  ...
  app.get("/api/diag/email", async (req, res) => {
    ...
    if (!apiKey) { d.result = "SKIP — BREVO_API_KEY not configured"; return res.json(d); }
    try {
      const payload = {
        sender: { name: fromName || "Renta Fácil TPymes", email: fromEmail || "noreply@rentafacil.es" },
        to: [{ email: testTo }],
        subject: "[DIAG] Test email Renta Fácil TPymes " + new Date().toISOString(),
        htmlContent: `<p>Email de prueba. Timestamp: ${new Date().toISOString()}</p>`,
      };
      ...
      d.brevo_response = body.substring(0, 500);
      d.result = resp.ok ? "OK — email enviado" : `FAIL ${resp.status}: ${body.substring(0, 200)}`;
    } catch (e: any) { d.result = `ERROR: ${e.message}`; }
    return res.json(d);
  });
  ...
  app.get("/api/diag/sheet", async (_req, res) => {
    ...
    try {
      ...
      const testRow = {
        ...
        environment: "test",
        created_at: _diagNow.toISOString(),
        updated_at: _diagNow.toISOString(),
        source_workflow: "diagnostico",
        cliente_nombre: "Test Diagnóstico",
        cliente_email: "test@diag.com",
        ...
        resultado_estimado: "364.81",
        tipo_resultado: "A ingresar",
        importe_resultado: "364.81",
        base_imponible_general: "22000",
        cuota_resultante: "2000",
        resultado_declaracion: "364.81",
        observaciones: "Fila de diagnóstico automático",
        legacy_timestamp: _diagNow.toLocaleString("es-ES", { timeZone: "Europe/Madrid" }),
        id_caso: `DIAG-${Date.now()}`,
        timestamp: _diagNow.toLocaleString("es-ES", { timeZone: "Europe/Madrid" }),
        nombreCompleto: "Test Diagnóstico",
        email: "test@diag.com",
    ...
    } catch (e: any) { d.result = `FAIL: ${e.message}`; }
    return res.json(d);
  });
