  ...
  guardarSimulacion: publicProcedure
    ...
    .mutation(async ({ input }) => {
      ...
      const _sheetRow: Record<string, unknown> = {
        ...
        // ── Campos legacy (columnas 98-153) para uniformidad con filas de n8n ──
        id_caso: _sheetExpId,
        ...
        realizaDonaciones: Array.isArray(datos.deducciones_check) && (datos.deducciones_check as string[]).includes("donaciones") ? "Si" : "No",
        tienePlanPensiones: Array.isArray(datos.deducciones_check) && (datos.deducciones_check as string[]).includes("planes_pensiones") ? "Si" : "No",
